  ...
  bot: async () => {
    try {
      ...
      client.on(
        ...
        async (message: {
          content: string;
          react: (arg0: string) => any;
          author: {
            bot: any;
          };
          channel: {
            send: (arg0: string | Discord.MessageAttachment) => void;
            type: string;
            bulkDelete: (arg0: number, arg1: boolean) => any;
          };
          ...
          if (command === 'ping') {
            ...
            **.play <number>** - Work in progress
            ...
            await VC.join().then(
              async (connection: { play: (arg0: string) => any }) => {
                if (args[0] === 'random') {
                  const playfile: any = fs.createWriteStream(
                    `app/resources/media/audios/random.mp3`
                  );
                  await axios({
                    method: 'get',
                    ...
                    const dispatcher: any = connection.play(
                      `app/resources/media/audios/random.mp3`
                    );
                  ...
                  const playfile: any = fs.createWriteStream(
                    `app/resources/media/audios/play.mp3`
                  );
                  await axios({
                    method: 'get',
            ...
            message.guild.members.cache.forEach(
              (member: {
                ...
                if (member.roles.cache.has(message.mentions.roles.first())) {
                  message.channel.send(
                    `${message.author} is moving users to a VC`
                  );
                  return member.voice.setChannel(channel);
                } else {
                  message.channel.send(
                    `${message.author} you need to specify a role`
                  );
                }
              }
            );
